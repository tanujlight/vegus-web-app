...
export class PlanListComponent implements OnInit {
  ...
  removePlan(event) {
    this.plansApi.delete(event.id).subscribe(res => {
      ...
      this.toasterService.success('', 'Plan deleted successfully')
    })
  }
  ...
  addPlan(event) {
    ...
    this.plansApi.add(sendData).subscribe(res => {
      if (res) {
        ...
        this.toasterService.success('', 'Plan added successfully')
      }
    })
  }
  ...
  updatePlan(event) {
    ...
    this.plansApi.update(event).subscribe(res => {
      if (res) {
        ...
        this.toasterService.success('', 'Plan updated successfully')
      }
    })
  }
}
@Component({
  template: `
    <input type="number" [ngModel]="cell.getValue()" (ngModelChange)="updateValue($event)" class="form-control" />
...
export class CustomNumberEditorComponent extends DefaultEditor implements OnInit {
  ...
  ngOnInit() {
    ...
    this.maxValue = this.cell.getColumn().editor.config?.maxValue || 100000 // Default to 100000 if not provided
  }
