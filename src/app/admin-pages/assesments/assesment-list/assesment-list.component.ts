...
import {Exam, ExamStatusListWithTitle} from '../assesments.interface'
...
export class AssesmentListComponent implements OnInit {
  ...
  onCustomAction(event): void {
    switch (event.action) {
      ...
      case 'edit':
        this.routeTo(`admin/exams/edit/${event.data.id}`)
        break

      case 'add':
        this.routeTo(`admin/exams/new`)
        break
          ...
          .onClose.subscribe((confirmed: boolean) => {
            if (confirmed) {
              this.examsApi.duplicateExam(event.data.id).subscribe(res => {
                this.routeTo(`admin/exams/edit/${res.id}`)
              })
            }
          })
        break
  ...
  private openViewExamDialog(exam) {
    this.dialogService.open(AssesmentViewComponent, {
      context: {
        // examId: exam.id
      },
