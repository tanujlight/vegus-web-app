...
import {CaseStudyViewComponent} from 'app/pages/case-studies/case-study-view/case-study-view.component'
import {QuestionViewComponent} from 'app/pages/questions/question-view/question-view.component'
...
export class QuestionsAndCaseStudiesComponent implements OnInit {
      ...
      question: {
        ...
        valuePrepareFunction: (question: any) => {
          if (!question || !question.title) return '--'
            ...
            return `<a href="/pages/questions/edit/${question._id}" class="text-primary">${question?.title}</a>`
          }
        }
      },
      caseStudy: {
        ...
        valuePrepareFunction: (caseStudy: any) => {
          if (!caseStudy || !caseStudy.title) return '--'
            ...
            return `<a href="/pages/case-studies/edit/${caseStudy._id}" class="text-primary">${caseStudy?.title}</a>`
          }
        }
      }
    }
  ...
  redirectToQuestionOrCaseStudyEditPage(event: QuestionAndCaseStudy) {
    if (event.type === 'question') {
      this.routeTo(`pages/questions/edit/${event.question.id}`)
    } else if (event.type === 'caseStudy') {
      this.routeTo(`pages/case-studies/edit/${event.caseStudy.id}`)
    }
  }
  ...
  remove(event: QuestionAndCaseStudy) {
    this.examsApi.deleteQuestionAndCaseStudy(this.examId, event._id).subscribe(res => {
      if (res) {
  ...
  add(newQuestionsAndCaseStudies: QuestionAndCaseStudy[]) {
    ...
    this.examsApi.addQuestionAndCaseStudy(this.examId, payload).subscribe(res => {
      if (res) {
        this.questionsAndCaseStudies = res.questionsAndCaseStudies
        this.totalMarksChange.emit(res.totalMarks)
